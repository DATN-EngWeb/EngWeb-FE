/* global requestAnimationFrame, fetch */
/* eslint-env browser */
...
class Stage {
  constructor(canvas) {
    ...
    canvas.addEventListener('mousedown', (e) => {
      pointerDown = true;
      this.dispatchEvent('pointerstart', { x: e.offsetX, y: e.offsetY, onCanvas: true });
    });
    canvas.addEventListener(
      ...
      (e) => {
        ...
        this.dispatchEvent('pointerstart', {
    ...
    canvas.addEventListener('mousemove', (e) => {
      if (pointerDown) {
        this.dispatchEvent('pointermove', { x: e.offsetX, y: e.offsetY, onCanvas: true });
      }
    });
    canvas.addEventListener(
      'touchmove',
    ...
    this.loop = (timestamp) => {
      ...
      this.dispatchEvent('ticker', frameTime, lag);
      lastTime = timestamp;
  ...
  dispatchEvent(evt, ...args) {
    if (this.listeners[evt]) this.listeners[evt].forEach((cb) => cb(...args));
  }
}
export function initFirework(refs, onComplete) {
  const trailsStage = new Stage(refs.trailsCanvas);
  const mainStage = new Stage(refs.mainCanvas);
  ...
  const appNodes = {
    stageContainer: refs.stageContainer,
    canvasContainer: refs.canvasContainer,
    controls: defaultNode,
    menu: defaultNode,
    menuInnerWrap: defaultNode,
    pauseBtn: defaultNode,
    pauseBtnSVG: defaultNode,
    soundBtn: refs.soundBtn,
    soundBtnSVG: refs.soundBtnSVG,
    shellType: defaultNode,
    shellTypeLabel: defaultNode,
    shellSize: defaultNode,
    shellSizeLabel: defaultNode,
    quality: defaultNode,
    qualityLabel: defaultNode,
    skyLighting: defaultNode,
    skyLightingLabel: defaultNode,
    scaleFactor: defaultNode,
    scaleFactorLabel: defaultNode,
    autoLaunch: defaultNode,
    autoLaunchLabel: defaultNode,
    finaleModeFormOption: defaultNode,
    finaleMode: defaultNode,
    finaleModeLabel: defaultNode,
    hideControls: defaultNode,
    hideControlsLabel: defaultNode,
    fullscreenFormOption: defaultNode,
    fullscreen: defaultNode,
    fullscreenLabel: defaultNode,
    longExposure: defaultNode,
    longExposureLabel: defaultNode,
    helpModal: defaultNode,
    helpModalOverlay: defaultNode,
    helpModalHeader: defaultNode,
    helpModalBody: defaultNode,
    helpModalCloseBtn: defaultNode,
  };
  ...
  const crackleShell = (size = 1) => {
    // favor gold less
    const color = Math.random() < 0.25 ? COLOR.Gold : randomColor();
    return {
      shellSize: size,
  ...
  function randomShellName() {
    ...
    return specialNames[(Math.random() * specialNames.length) | 0];
  }
  ...
  const fastShellBlacklist = ['Strobe', 'Willow'];
  ...
  function startSequence() {
    if (isFirstSeq) {
      ...
      const leftPositions = [
        { type: horsetailShell, x: 0.18, y: 0.15 }, // 1 Horse Tail top
        { type: palmShell, x: 0.08, y: 0.25 }, // 2 Palm near horse tail
        { type: palmShell, x: 0.28, y: 0.25 },
        { type: crackleShell, x: 0.12, y: 0.45 }, // 1 Crackle
        { type: crossetteShell, x: 0.25, y: 0.55 }, // 1 Crossette
        { type: floralShell, x: 0.15, y: 0.7 }, // 1 Floral
        { type: ringShell, x: 0.08, y: 0.85 }, // 2 Rings bottom
        { type: ringShell, x: 0.28, y: 0.85 },
      ];

      const rightPositions = leftPositions.map((p) => ({
        type: p.type,
        x: 1 - p.x,
        y: p.y,
      }));
      ...
      // Wait 800ms for the Animate.css text to crash down
      setTimeout(() => {
        allPositions.forEach((pos) => {
          // Add a tiny random variance so it's not perfectly robotic
          setTimeout(() => {
            const config = pos.type(1);
            // Reduce spread size slightly so they don't cover the entire screen and overlap into a mess
            config.spreadSize = (config.spreadSize || 300) * 0.7;
            const shell = new Shell(config);

            // Fool the burst method into thinking it had a comet so it plays the sound and inherits speeds
            shell.comet = { speedX: 0, speedY: 0 };

            // Burst directly at pixel coordinates (bypassing the launch phase entirely)
            shell.burst(pos.x * w, pos.y * h);
          }, Math.random() * 300);
        });
      }, 800);

      // Wait 6.5s total before triggering redirect
      setTimeout(() => {
  ...
  function handlePointerEnd(_event) {
  ...
  function updateGlobals(timeStep, lag) {
    ...
    if (store.state.config.autoLaunch) {
      ...
      if (autoLaunchTime <= 0) {
        autoLaunchTime = startSequence() * 0.5;
      }
    }
  }
  ...
  function colorSky(speed) {
    ...
    appNodes.canvasContainer.style.backgroundColor = `rgba(${currentSkyColor.r | 0}, ${currentSkyColor.g | 0}, ${currentSkyColor.b | 0}, 0.3)`;
  }
  ...
  const soundManager = {
    ...
    playSound(type, scale = 1) {
      ...
      // Disallow starting new sounds if sound is disabled, app is running in extremely slow motion (less than 0.2), or paused.
      if (!canPlaySoundSelector() || simSpeed <= 0.2) {
        return;
      }
  ...
  // Start app immediately to avoid black screen while fetching sounds from remote
  init();
  soundManager.preload().catch((e) => {
  ...
  return {
    destroy: () => {
      trailsStage.destroy();
      mainStage.destroy();
      refs.soundBtn.removeEventListener('click', toggleSound);
    },
  };
}
